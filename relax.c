...

{
  ...
  for (i = 0; i < current_goals->num_F; i++)
  {
    for (j = 0; j < S->num_F; j++)
    {
      /*因为fact都用整值进行替换，只需要判断=即可*/
      if (S->F[j] == current_goals->F[i])
        break;
    }
    if (j == S->num_F)
    {
...
创建并初始化一些数据结构和变量，如目标状态 lcurrent_goals、时间图 lU、RP（reasoning process）相关的变量等。
创建并初始化用于推理过程中的 SAT 公式。该公式用于表示初始情况下的已知条件和目标状态，以便后续的推理可以基于这些信息进行。
如果启用了相关的调试选项，函数会输出一些调试信息，以便在开发过程中可以查看和验证算法的正确性。
*/
...

{
      ...
      gcmd_line.heuristic == 1)
  {
     ...
     * 注意: 按长度递增对子句进行排序; 这样，稍后，当我们从前到后处理子句时，以查看其中是否包含叶子分离时，我们将首先尝试最短的子句（请参见下面的rplan提取）
     */
    ...
    for (i = 0; i < gnum_initial_or; i++)
    {
      ...
      if (gcmd_line.R && gcmd_line.debug)
      {
        printf("\nr inserted OR clause");
        print_r_clause(lr_num_clauses - 1);
        fflush(stdout);
      }
    } /* for all initial ors */
...

{
  ...
  for (i = lr_num_clauses - 1; i >= 0; i--)
  {
    for (j = 0; j < lr_clause_length[i]; j++)
    {
      ...
      { /* literal negative; do the same, basically */
...

{
  ...
  if (ehc_father)
  {
    ...
    else
    {
      ...
      for (iehc = ehc_father; iehc->father; iehc = iehc->father)
      {
        ...
        {
          printf("\n--- op: ");
          print_op_name(iehc->op);
          printf("\n-----ehc father state %d", num_path + 1);
          print_state(*path[num_path + 1]);
          fflush(stdout);
        }
      }
      ...
      for (i = gnum_plan_ops; i >= 0; i--)
      {
        ...
        if (gcmd_line.R && gcmd_line.debug)
        {
          ...
          printf("\n-----ehc father then gplan state %d", num_path + 1);
          print_state(*path[num_path + 1]);
          fflush(stdout);
        }
      }
  ...
  if (bfs_father)
  {
    ...
    for (ibfs = bfs_father; ibfs; ibfs = ibfs->father)
    {
      ...
      if (gcmd_line.R && gcmd_line.debug)
      {
        ...
        {
          printf("\n--- op: ");
          print_op_name(ibfs->op);
        }
        printf("\n-----bfs state %d", num_path + 1);
        print_state(*path[num_path + 1]);
        fflush(stdout);
      }
    }
    ...
    for (i = gnum_plan_ops; i >= 0; i--)
    {
      ...
      */
      ...
      if (gcmd_line.R && gcmd_line.debug)
      {
        ...
        printf("\n-----not ehc not bfs state %d", num_path + 1);
        print_state(*path[num_path + 1]);
        fflush(stdout);
      }
    }
  ...
  for (t = num_path + 1; t < MAX_PLAN_LENGTH; t++)
  {
    ...
    for (i = 0; i < lnum_U[time]; i++)
    {
      ...
      {
        if (lU[time][i].ft == lU[time + 1][j].ft)
          break;
      }
      if (j == lnum_U[time + 1])
        continue;
  ...
  for (t = num_path + 1; t < MAX_PLAN_LENGTH; t++)
  {
    ...
    for (i = 0; i < path[t + 1]->num_unknown_E; i++)
    {
      ...
      {
        if (lU[time][k].ft == gef_conn[ef].C[c])
          break;
      }
      if (k == lnum_U[time])
      {
      ...
      {
        if (!Utpp[gef_conn[ef].A[j]])
          continue; /* known in next step */
        ...
        {
          if (lU[time + 1][k].ft == gef_conn[ef].A[j])
            break;
        }
        /* DEBUGGING
         */
...

{
  ...
  for (i = 0; i < lnum_U[Utime]; i++)
  {
    ...
    for (j = 0; j < gft_conn[ft].num_C; j++)
    {
      ...
      if (gcmd_line.R && gcmd_line.debug >= 3)
      {
        printf("\nadded 1 to val of ef %d, now %d, fact ", ef, Ups[ef]);
        print_ft_name(ft);
        fflush(stdout);
      }
      if (gef_conn[ef].num_active_PCs + Ups[ef] == gef_conn[ef].num_PC)
      {
        ...
        for (k = 0; k < gef_conn[ef].num_C; k++)
        {
          ...
          {
            if (gef_conn[ef].C[k] == lU[Utime][conu].ft)
              break;
          }
          if (conu < lnum_U[Utime])
            break;
        }
        /* DEBUGGING, REMOVE LATER.
         */
        ...
        for (k = 0; k < gef_conn[ef].num_A; k++)
        {
          ...
          {
            if (gef_conn[ef].A[k] == lU[Utime + 1][addu].ft)
              break;
          }
          ...
          else
          {
            ...
            if (l == lU[Utime + 1][addu].num_in)
            {
              if (lU[Utime + 1][addu].num_in == MAX_UEDGES)
              {
                ...
                {
                  printf("\nedge to: ");
                  print_ft_name(lU[Utime + 1][addu].in_edges[k]->ft);
                  printf(" by ");
  ...
  if (!new_U)
  {
    ...
    for (i = 0; i < lnum_U[Utime + 1]; i++)
    {
      ...
      {
        if (lU[Utime][j].ft == lU[Utime + 1][i].ft)
          break;
      }
      /* DEBUGGING; REMOVE LATER
       */
  ...
  for (t = Utime + 1; t > 0; t--)
  {
    ...
    for (i = prev_p; i < now_p; i++)
    {
      ...
      for (j = 0; j < p[i]->num_in; j++)
      {
        ...
        {
          printf("\nr new p: ");
          print_ft_name(p[i]->in_edges[j]->ft);
          printf(" (by ");
          if (p[i]->in_efs[j] == -1)
          {
            printf("NOOP ");
...

{
  ...
  for (i = 0; i < lr_num_clauses; i++)
  {
    ...
    for (j = 0; j < lr_clause_length[i]; j++)
    {
      ...
      {
        if (lr_clauses[i][j].literal == cdis[k])
          break;
      }
      if (k == num_cdis)
      {
...

{
  ...
  if (!gop_conn[op].is_used_at[time - 1])
  {
    ...
    if (!gop_conn[op].is_in_H)
    {
      ...
      {
        if (gft_conn[gop_conn[op].P[i]].level != 0)
          break;
      }
...

{
  ...
  for (j = 0; j < gef_conn[ef].num_PC; j++)
  {
    ...
    { /* fact is not known at the effcond level */
      /* DEBUGGING! REMOVE THAT LATER
       */
      for (i = 0; i < lnum_U[Utime - 1]; i++)
      {
        if (lU[Utime - 1][i].ft == ft)
          break;
      }
      if (i == lnum_U[Utime - 1])
      {
    ...
    for (j = 0; j < gop_conn[H[i]].num_E; j++)
    {
      ...
      for (k = 0; k < gef_conn[ef].num_D; k++)
      {
        if (gft_conn[gef_conn[ef].D[k]].is_goal_at[0] ||
            gft_conn[gef_conn[ef].D[k]].is_goal_at[1])
          d++;
      }
    }
    for (j = 0; j < gnum_H; j++)
...

{
  ...
  if (gcmd_line.R && gcmd_line.debug >= 2)
  {
    ...
    {
      print_ft_name(leafs[min_leafs[i]]);
      printf(" ");
    }
    fflush(stdout);
  }
...

{
  ...
  for (i = 0; i < num_dis; i++)
  {
        ...
        is_dis[gft_conn[dis[i]].negation])
    {
      ...
      for (k = 0; k < num_dis; k++)
      {
        if (gft_conn[dis[i]].negation == dis[k])
          break;
      }
      if (k == num_dis || k == i)
      {
  ...
  if (num_cdis > 1)
  {
    ...
    while (curr_test_index < num_cdis)
    {
      if (gcmd_line.R && gcmd_line.debug >= 2)
      {
        ...
        {
          printf("-");
          print_ft_name(gcf[(-1) * cdis[curr_test_index]]);
          printf("(%d)", lStime);
        }
        else
        {
...

{
  ...
  if (gcmd_line.debug)
  {
    ...
    for (i = lr_num_clauses - 1; i >= 0; i--)
    {
      for (j = 0; j < lr_clause_length[i]; j++)
      {
        v = (lr_clauses[i][j].literal > 0) ? lr_clauses[i][j].literal : (-1) * lr_clauses[i][j].literal;
        if (sum == -1 || v > sum)
        {
          sum = v;
        }
...

{
  ...
  for (t = num_path + 1; t < MAX_PLAN_LENGTH; t++)
  {
    ...
    for (i = 0; i < path[t + 1]->num_unknown_E; i++)
    {
      ...
      for (j = 0; j < gef_conn[ef].num_A; j++)
      {
        if (!gft_conn[gef_conn[ef].A[j]].CNF)
          continue;
        if (Ftpp[gef_conn[ef].A[j]])
          continue;
        if (gef_conn[ef].A_nondet[j])
          continue;
      ...
      {
        if (!gft_conn[gef_conn[ef].D[j]].CNF)
          continue;
        if (!Ftpp[gef_conn[ef].D[j]] && !Utpp[gef_conn[ef].D[j]])
          continue;
...

{
  ...
  for (i = 0; i < gop_conn[op].num_E; i++)
  {
    ...
    for (j = 0; j < gef_conn[ef].num_C; j++)
    {
      if (!Ft[gef_conn[ef].C[j]])
        break;
    }
    if (j < gef_conn[ef].num_C)
      continue;
