...

{
  ...
  for (i = 0; i < gop_conn[op].num_E; i++)
  {
    ...
    {
      if (F[gef_conn[ef].C[j]])
        fcount++;
      if (U[gef_conn[ef].C[j]])
        ucount++;
...

{
  ...
  for (i = 0; i < dest->num_unknown_E; i++)
  {
    ...
    {
      if (gef_conn[ef].A[k] == ft)
        break;
    }
    if (k == gef_conn[ef].num_A)
      continue;
...

{
  ...
  for (i = 0; i < num_cn; i++)
  {
    ...
    if (gft_conn[check_neg[i]].negation != -1)
    {
      ...
      {
        if (dest->F[j] == gft_conn[check_neg[i]].negation)
          break;
      }
      if (j < dest->num_F)
      {
      ...
      {
        if (dest->U[j] == gft_conn[check_neg[i]].negation)
          break;
      }
      if (j < dest->num_U)
      {
...

{
  ...
  {
    printf("\n-----op ");
    print_op_name(path_op[num_path + 1]);
  }
  if (ehc_source)
  {
...

{
  ...
  int i, j, ft, time;
  Bool neg;
  /*printf("%d------%d",gnum_fixed_c,(1 + MAX_PLAN_LENGTH) * gmax_CNFU + 1);*/
...

{
  ...
  if (gcmd_line.debug)
  {
    ...
    for (i = gnum_clauses - 1; i >= 0; i--)
    {
      for (j = 0; j < gclause_length[i]; j++)
      {
        v = (gclauses[i][j].literal > 0) ? gclauses[i][j].literal : (-1) * gclauses[i][j].literal;
        if (sum == -1 || v > sum)
        {
          sum = v;
        }
