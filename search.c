 ...
 * encountered by current serial plan - DISACTIVATED IN CONFORMANT SETTING...
 当前串行计划中已经遇到的状态的内存（哈希表），在conformant设置中被禁用
 */
...

{
  ...
  if (first_call)
  {
    /* on first call, initialize plan hash table, search space, search hash table
     在第一次强制爬山时，初始化plan 哈希表，搜索空间，搜索哈希表
     */
...

{
  ...
  for (i = 0; i < gnum_H; i++)
  {
     ...
     */
    if (!result_to_dest(&S__, tmp, NULL, gH[i]))
      continue;
...

{
  ...
  for (i = 0; i < gnum_H; i++)
  {
    if (!result_to_dest(&S_, lehc_current_start, NULL, gH[i]))
      continue;
...

{
  ...
  while (TRUE)
  {
    ...
    for (i = 0; i < gnum_A; i++)
    {
      if (!result_to_dest(&S, NULL, first, gA[i]))
        continue;
...

{
  ...
  for (i = lbfs_space_head; i->next; i = i->next)
  {
    ...
    {
      if (i->next->h > h)
        break; /* stop only when next one is strictly worse */
    }
    else
    {
      if (i->next->h >= h)
        break; /* stop already when next one is equally good */
    }
  }
